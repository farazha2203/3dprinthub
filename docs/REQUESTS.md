...
Status: `A2W W1/W2/W3/W4 WINDOWS_RUNTIME_ACCEPTED / W4.1 LOCAL_TESTED / REAL DIMENSION PATCH NEXT`.
...
Follow-up A2W scope records source video import for Site/Instagram, factual multi-Profile extraction from the Product link, material-family mapping and a manual/self-produced Product flow. W3 and W4 are runtime-accepted on real #625. W4 real Preview returned 2 Profiles / 3 Source slots / 48 Local PLA candidates / exact HEX=0 without mutating Source/Ledger/locks/Site identity. W4.1 is now Local-tested: exact Source Description dimensions are preserved per profile, partial factual axes stay partial/visible as `نامشخص`, and the owner-approved #625 4×4×4cm estimated fallback may fill only still-missing axes. Real Hydra #628 proves Small=12cm height and Large=18cm height from the latest MakerWorld capture.
