...
Real diagnosis: no current #609 Instagram receipt/post exists. Buffer is connected and github_raw is configured. The failure occurred before provider submission because Story preparation still required an unnecessary Site FTP derivative upload and received WinError 10054. ERR-49-219 removes that dependency only for github_raw while retaining Site mode. Real #609 GitHub rehost now passes public MIME/SHA 6/6. Before the one real send, the clickable Story contract is also hardened so Buffer uses notification-mode with an explicit UTM Link Sticker handoff and records notification-ready rather than falsely claiming a live Story; this is Local-tested 36/36.
