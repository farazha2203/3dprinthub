...
Status: `A2T ACCEPTED / GITHUB PUSHED / WINDOWS SHORTCUT VERIFIED`.
...
A2T preserves the live A2M authoritative server contract and re-verifies both Windows and Server republish suites. Instagram Feed stays automatic but no longer pretends a caption URL is clickable; Story clickable-link mode uses Buffer notification publishing so the owner completes the native Link Sticker in Instagram with label «لینک محصول». #625 revision 8 remains duplicate-protected and was not reposted. The A2T source was pushed at `df38414e...`; the old Desktop launchers were backed up and both `.lnk`/`.cmd` are now retargeted to the verified A2T launcher with real launch-smoke PASS.
