...
O1 Product status filters + split Instagram Post/Story -> O1B recent edited/viewed Product activity -> O2 Crawl complete/incomplete -> O2D Product identity/dedup/consumed-Crawl suppression -> O2E Product Media Truth/Refresh/Instagram image parity -> O2G target-aware Search Crawl pagination -> O2H guarded hard delete for broken Crawl identity -> O2F Instagram disclosure/source+order links/Story link -> O3 guarded deep reset/refetch/remap -> O4 Delete semantics -> O5 Social receipt/operator acceptance -> O6 integrated regression/closure.

O1, O1B, O2, O2D and O2E are ACCEPTED. O2E final source is GitHub-exact at `544e046b...`: Product Editor/Refresh/Instagram share one exact selected Product-local media authority; Refresh is compare-only; final focused 16/16, direct Instagram 13/13, expanded Image/Product/Social 131/131 and static/framework gates PASS. Fresh rollback/integrity, graceful exact-SHA cutover and real #625/#628 ProductWizard/Refresh/Social parity all PASS with unchanged target Product/History digest. Exact next is O2G target-aware MakerWorld search pagination, then O2H guarded hard delete, O2F Instagram disclosure/links, then O3.
...
## 2026-09-19 - Phase50.A.2M authoritative existing-Product re-publish
Product #625 / Site #39 exposed a false-success compatibility gap: the latest Windows profile/media revision imported in-place, but stale legacy EP49-3F Variants stayed active and could still drive public weight/material/price. A2M defines re-publish as replacement of the active Product configuration while preserving Product identity and historical rows inactive. Exact current ProductImage selection remains authoritative, explicit Windows local-image add/select is added, and sync fails closed if active Variant state is not exclusively the current CC matrix. Local Server 15/15 + Windows image 25/25 and no-drift gates PASS. Next: commit/push -> selective Production release -> fresh rollback -> reverse-tunnel deploy -> single #625 re-publish/browser verification.
...
## 2026-09-14 - ??? ???? ????: Orderable Publish Contract
????? ??? ?? HTTP 200 ?? Variant ????/???????? ????? ????? ????? ???????. QA ????? #62/#84 ???? ??? ???? Profile canonical ??????? Product ????? ???? ??? Cart ?????? ??????? ????. ??????? ???? ??? ?? FTP ???? Profile canonical ?? ?????? ?????? ? Host ??? ???? visibility ???? ?????? ?? ????? ?? Variant ?? ???? ????? ????? API ????? orderable ????. ??????? Local ? no-migration PASS ?????. ??? ???? commit/push? Deploy ????????? ?? baseline `6569e5a...`? ??? bootstrap/republish #62/#84 ? Cart acceptance ???. #628/#634 ???????? ????-ready ?????? #43 ?? ????? ???? ????? Material/Color ????? ???????.

## 2026-09-14 - ??? ???? ????: ?????????? ???? ???? ??? Windows ? Host

?????? ????? ?? ????? #628 ? #634 ???? ??? Windows ??? ????? ???? ???? `source_license_owner_approved=1` ?? ???? ????????? ????? ?? ??? ???? ???? ???????? ??? Host ??? `commercial_status` ??? ?? ????? ??????. ????? Local ???? ??? ?? ????? `commercial_status=review`? ???? Authority ?? ?? importer? ????? Product ? visibility ??????? ????? ?????? ? ?? ???? Owner approval ?????? fail-closed ???. ??????? Site/Catalog/no-migration ? runner ??????? PASS ?????. ??? ????: commit/push? Deploy ???? Migration ?? Production `44a7be9...` ? ??? retry ??? ???? ?? ????? ?? strict ACK ? Cart verification.
...
Authenticated Production audit PASS: Python 3.12.13, Django 6.0.7, correct MySQL database, Store 0036-0042 + Website 0024 applied, empty migration plan, 13 active Materials, 5 active PrintQualities, healthy storage/schema, and `publish_readiness.ready=True` with no blockers. Public Home/Store/Bridge/readiness and the A2G JS/CSS are HTTP 200. Phase50.A.2G is `PRODUCTION_VERIFIED`; the remaining acceptance gate is exactly one controlled real Product Windows -> Site publish with SEO WebP, strict ACK, canonical Product/Profile/Variant, public page/media and cart verification before bounded bulk.
