...
- [x] O6A1/O6A1B LOCAL_TESTED: Stage-3 stale media cards/current-file byte reload fixed; blue refresh is DB/Local-only; historical successful Buffer automatic Story route restored and refined to carry the tracked current 3DPrintHub Product URL in `metadata.instagram.link`; native mobile-notification sticker remains explicit opt-in only. Current focused 55/55 + related 74/74 PASS; static/Qt gates PASS; #862 canonical media parity remains 4 DB / 4 selected / 4 current-local / 4 visible cards. No real publish/deploy.
- [ ] O6A1B GitHub + exact-SHA runtime acceptance NEXT, then STOP.
- [ ] O6B: full exact-SHA runtime/UI smoke after O6A1 closure. STOP.
