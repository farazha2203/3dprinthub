...
- [ ] O2G LOCAL TESTED / RUNTIME ACCEPTANCE NEXT: target-aware MakerWorld discovery is implemented and regression-green. Real cloned `Lamp` probe progressed from the old 15-row window to scroll depth 48, saw 397 links and persisted exactly the requested 200 unconsumed candidates; HTTP 403 correctly fell through to documented Chrome 9222 rather than false exhaustion. Focused 49/49, Preview 3/3, related 98/98 and static/framework gates PASS. Fresh Catalog rollback/integrity PASS. Remaining gate is commit/push -> Local=Remote -> exact-SHA Qt foreground/runtime acceptance.
- [ ] O2H NEXT AFTER O2G: implement guarded hard-delete for broken unconsumed Crawl identities; current Reject only changes status and is not deletion. Delete Crawl/Candidate/Preview/cache/verified candidate-derived data after rollback; Product-backed identities fail closed.
