...
- [ ] O2E CORRECTIVE_LOCAL_TESTED: initial GitHub checkpoint `a975eaaa...` exposed real #625 selected-card omission; later #628 Refresh exposed ASCII-safe Server rename not recognized by basename-only parity. Corrective card synthesis + finalized-SHA Site representation are Local-tested: focused 16/16, direct Instagram publish 13/13, expanded Image/Product/Social 131/131, compile/Qt VerifyOnly/Django/no-drift/diff-check PASS. Final corrective commit/push + fresh rollback + exact-SHA runtime acceptance remain.
- [ ] O2G: make MakerWorld lazy/infinite-scroll discovery target-aware so requested 200/300 drives progressive scrolling until target reached or verified exhaustion; fixed Preview scroll depth must not cap results around the first ~15 cards.
- [ ] O2H: implement guarded hard-delete for broken unconsumed Crawl identities; current Reject only changes status and is not deletion. Delete Crawl/Candidate/Preview/cache/verified candidate-derived data after rollback; Product-backed identities fail closed.
- [ ] O2F: verify current Buffer/Meta capabilities and implement supported AI disclosure label, source/order Product details links and Story link workflow.
