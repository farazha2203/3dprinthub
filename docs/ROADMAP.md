...
- [ ] O2H LOCAL_TESTED / GITHUB CHECKPOINT NEXT: guarded hard-delete is implemented separately from reversible Reject for unconsumed Crawl identities. Candidate/Preview/cache/canonical candidate-derived local data are bounded to the exact identity; Product authority by external ID or normalized URL fails closed. Focused 5/5, related 68/68 and static/framework gates PASS; fresh rollback/integrity backup PASS. Commit/push + exact-SHA clone/runtime acceptance remain before ACCEPTED.
