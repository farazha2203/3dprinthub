...
- [x] O6A1 LOCAL_TESTED: runtime regression hotfix for Stage-3 stale media cards/current-file byte reload + DB/Local-only blue refresh + restore proven absent-setting Story default `bio_shop_grid`; changed-condition 31/31 and related 84/84 PASS; live #862 read-only 4/4/4 parity and Buffer Story readiness ready=true without mobile handoff. No write/publish/deploy.
- [ ] O6A1 GitHub + exact-SHA isolated runtime acceptance NEXT, then STOP.
