...
- [x] Fail closed before fetch on Source URL/external-ID mismatch or operator-owned Product-local media.
- [x] Create/verify per-repair full Catalog SQLite rollback before any Product mutation.
- [x] Acquire into a unique empty same-identity folder and delete only failed partial fresh output on acquisition failure.
- [x] Reverify refreshed Source identity/local_dir and preserve/remap existing selected Source media by stable asset identity; fail if selection disappears.
...
- [x] O2D ACCEPTED: GitHub-exact source `4e667f4f...`; existing Product identities suppressed from Add Products/live candidates/batch pending/Preview/legacy queue and queue summary; anti-recrawl ledger retained; Product table/detail exposes `source_code:external_id`; exact-SHA runtime truth 1192 raw Crawl = 707 consumed hidden + 485 visible (50 ready / 435 incomplete), visible mapped Products=0, canonical duplicate groups=0, no-mutation digest/rollback/runtime smoke PASS.
...
- [x] O5C LOCAL_TESTED: current fingerprint mandatory; Feed/Story independent; provider-ID-first; unique asset fallback only; missing/ambiguous/non-sent fail closed; stale current-UI filters fixed; no repost. Focused Social/O5C 17/17 PASS and canonical read-only filter proof excludes stale #536/#625.
- [x] O5D ACCEPTED: related Social/Image/duplicate-guard 77/77 PASS after stale O1 fixture correction; py_compile/compileall, pip check, diff-check, Qt VerifyOnly, Django check and migration drift PASS; GitHub-exact commit `df4e9a53fb33ca4a8c25282503f75d9fce7f9473`, Local=Remote, clean worktree; no Catalog/provider mutation and no Production deploy.
- [x] O5E ACCEPTED: fresh verified rollback `phase50-a2z-o5e-20260926-145851`; isolated clone reconciliation appended exactly six current provider-sent final receipts (477->483), exact provider-ID/no-repost only; 21 non-receipt tables exact; #536/#625/#609/#588 controls exact; #301/#588 Story still missing/current-fail-closed; #609 current Story remains provider error; canonical all-table digests unchanged and quick_check=ok.
- [x] O5F ACCEPTED: fresh verified rollback `phase50-a2z-o5f-20260926-150923`; live exact-ID provider preflight = six `sent` + #609 Story `error`, zero mutations; canonical Receipts 477->483 with exactly six exact-ID/no-repost finals; 21 non-receipt tables exact; controls/stale/missing Story truth preserved; 77/77 related regression + Qt VerifyOnly + exact-SHA visible PID 57184 PASS; no Production deploy.
- [x] O5G NOT_REQUIRED / external gaps preserved: owner explicitly chose O6A; no real provider action was auto-run. #609 clickable Story remains blocked by Buffer mobile readiness and #301/#588 lack current Story evidence.
- [x] O6A ACCEPTED: integrated Product/Crawl/Media/Social 224/224 PASS; compileall/pip/diff/Qt VerifyOnly PASS; canonical integrity PASS with zero identity/lifecycle/link/Social-final violations and zero strict media SHA/path errors. 475 legacy selected-media Products remain missing exact Local files as known fail-closed repair debt. Concurrent Qt ingestion changed Product count independently; receipts/crawl/candidates stayed stable.
- [x] O6A1/O6A1B ACCEPTED: Stage-3 stale media/current-byte display fixed; blue refresh is deterministic DB/Local-only; historical automatic Buffer Story restored with tracked Product URL in `metadata.instagram.link`, native mobile notification opt-in only. Focused 55/55 + related 74/74 + static/Qt gates PASS; exact Product #862 refresh/reopen = 4/4/4/4 with valid pixmaps/SHA parity and zero DB count drift; live Story readiness ready=true without mobile handoff; source commit `2f11c225...`, exact-SHA visible PID 47280 PASS. No real publish/deploy.
- [x] O6B ACCEPTED: exact application-source Qt VerifyOnly + responsive single runtime PID 51712; isolated owner-visible UI contract proves Products/Crawl, DB/Local media refresh, full reacquire, guarded hard-delete, independent Instagram Post/Story actions and current-revision Social filters. #862 remains stable 4/4/4/4 with valid pixmaps after refresh; live Story readiness ready=true without mobile handoff; canonical post-smoke quick_check=ok with zero duplicate/Product-backed-Crawl/current-Social-final violations. No write/publish/deploy.
- [x] O6C ACCEPTED: Local/upstream/live GitHub exact `d2838684...`; last verified Production `2b48a593...` -> O baseline `82862b45...` -> current HEAD ancestry PASS. Full Production-to-HEAD delta includes historical pre-O Server files, but exact O-baseline-to-HEAD delta is only 44 Catalog + 8 docs with zero Server/static/migration/Host-script delta. Reverse tunnel 22024 is currently unavailable read-only, so fresh Host HEAD is not asserted. Decision = NO DEPLOY REQUIRED for O phase.
- [x] O6D SKIPPED / NOT_REQUIRED: O phase carries no Production application delta; do not deploy unrelated pre-O ZarinPal/A2R/A2Z Server files under O closure.
- [ ] O6E NEXT: final documentation closure + final Local=Remote proof; preserve external/tunnel notes; STOP.
