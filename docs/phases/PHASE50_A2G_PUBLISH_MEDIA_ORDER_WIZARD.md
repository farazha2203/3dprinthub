...
## 2026-09-15 Client handoff Store Reset + Hero delivery extension
A2G client-handoff scope now includes a rollback-grade empty-Store operation plus the pending visible Hero replacement. The Store Reset route is authenticated and fail-closed: only fully imported Catalog Products are eligible, protected commerce/inventory/manual Product states block mutation, and source assets/master data/Portfolio/Hero/history are preserved. A new repository-owned Host helper prepares exact live-count JSON plus checksum-identical Product media; a fresh verified real MySQL gzip remains mandatory before reset. Hero candidate 50.3.0 is full-bleed on desktop and uses real 3D cuboid faces with randomized vertical/horizontal 3/5/7-slice transitions, while mobile/reduced-motion preserve the mature fallback and server-rendered content stays authoritative. Local Store Reset+Hero 8/8 and broader Qt/Publish/Instagram/SiteConnection 57/57 PASS; no migration drift and migration plan is empty. Production remains unchanged until the candidate is committed/pushed, Host transport/state are reverified, backups are created and reset preflight is eligible.
...
## 2026-09-14 owner-driven republish preparation
Local Catalog publication setup was simplified for manual owner operation. All active Filament inventory now has one 1000 g roll and nonzero commercial/service pricing; every current Product has a canonical Profile with all unique active Filament identities preselected. Existing factual profile data is retained and only missing production rows receive the mature fallback facts. Previously uploaded Products are update candidates, not duplicate-publish candidates.

Local rollback: `D:\projects\3dprinthub-backups\phase50-filament-defaults-20260914-175618`. Focused Filament/Profile suites 34/34 and Qt verify-only PASS. The owner now controls which Products are published from the relaunched Windows app. This checkpoint does not claim the separate Production Store Reset is deployed or complete.
