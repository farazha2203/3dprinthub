...
## Exact next
Final O2F diff/staging review -> commit/push tested source/tests/checkpoint docs -> verify Local=Remote -> exact-SHA Qt runtime acceptance -> read-only real Product smoke proving all-current Feed authority count can exceed selected count without Catalog mutation. Do not publish an arbitrary Product merely as a test. Real clickable Story acceptance remains blocked only by Buffer mobile readiness. Then O3 guarded deep reset/refetch/remap; O4 follows after O3.
