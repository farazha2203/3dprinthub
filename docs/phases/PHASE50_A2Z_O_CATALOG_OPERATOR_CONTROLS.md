...
- **O4R3 — isolated real acceptance/runtime — IN_PROGRESS:** first fresh clone real Sphinx `makerworld:2737114` passed adaptive `rich -> network_capture`, Product creation, 5 local images, 2 file links, Source description, screenshot, Crawl collected/live hidden and quick_check, with canonical unchanged; however the real page acquisition left `source_print_profiles_json=[]`. The narrow correction now invokes the existing factual MakerWorld `refresh_source_profiles(fresh_capture=True)` before `import_source_profiles`; focused + related including A2W Source Profiles is 74/74 PASS. Next: commit/push this correction/docs -> fresh rollback/clone -> exact-SHA real retest requiring Profile/Filament evidence when available -> Qt visible smoke -> only then bounded canonical owner-visible acceptance. STOP.
