...
**Safety evidence:** FTP completed 12/12 but Bridge imported 0/failed 1; Local and Site Product #38 remain revision 1, `needs_update=1`, Catalog quick_check=ok. The importer transaction rolled back, and no third unchanged retry is allowed.
**Correction to ERR-49-234:** canonical ASCII Server/Public destination basenames are correct hardening but were not sufficient. The importer must first open the uploaded Batch source path before it can canonicalize the destination.
**Root cause proof:** GitHub-exact rollback-only tracer `6d0dbbe3...` ran the failed Batch `desktop_catalog_v85_20260924_105415` through every importer stage under the normal UTF-8 CLI process: category/source/upsert/reconcile/images/videos/Product/Phase39/Phase43/visibility/parity/Portfolio all PASS; parity was `ok=true, media_count=2, profile_count=32`; forced DB rollback left Site Product #38 at revision 1 and Production clean. Then the same Batch was read-only opened with `LC_ALL=C LANG=C PYTHONUTF8=0 PYTHONCOERCECLOCALE=0`: filesystem encoding became `ascii`, `local_image_files_json[0]` was non-ASCII, and `Path.open()` reproduced the Production exception exactly, including `positions 127-131`.
**Correct fix:** GitHub-exact Windows code commit `7b1b447ab772e15f3ecdc1345e69e2e8771573f8` makes Batch packaging keep the operator-approved Unicode SEO filename only in `image_metadata_json`, but copies/uploads selected media under the deterministic ASCII-safe Server basename and writes those ASCII names to `local_image_files_json`. Existing ASCII filenames stay unchanged. Windows helper and live Server canonical helper produce the exact same #628 basename `majestic-hydra-voronoi-art-sculpture-3d-print-01.webp`.
**Verification:** focused new boundary regression 1/1 PASS; bulk-publish + batch-packaging 33/33 PASS; broader V84/bidirectional/Crawl 50/50 PASS; py_compile/diff/Qt VerifyOnly PASS. A real #628 package dry-run on an online SQLite clone produced exactly two ASCII Batch filenames, preserved the Persian metadata filename and image SHA order, and left the canonical Catalog logical SHA/state byte-logically unchanged. Evidence root: `D:\projects\3dprinthub-backups\phase50-a2z-628-package-dryrun-20260924-111133`.
**Prevention:** every physical filename crossing Windows -> FTP -> shared-host Batch storage must be ASCII-safe before upload. Unicode is allowed in editorial/SEO metadata, ALT/caption and database text, but not in Batch source path identity. Destination canonicalization alone is too late.

## ERR-49-234 - Product #628 import rolled back on Unicode Server/Public media basename
**Date:** 2026-09-24
**Observed:** bounded Batch `desktop_catalog_v85_20260923_230223` for Desktop Product #628 reached FTP and Bridge import, then returned `UnicodeEncodeError: 'ascii' codec can't encode characters...`. The transaction reported `published=0 / failed=1`; Local stayed Site Product #38 revision 1 with `needs_update=1`.
**Safety evidence:** fresh Production DB+full-media rollback `/home/sfkilvrs/3dprinthub-deploy-backups/20260923-230201-a2z-628-prepublish` had already passed gzip/tar/SHA verification. Import is wrapped in `transaction.atomic`, so the failed Product/Portfolio conversion did not advance Site revision. No unchanged retry was attempted.
**Root cause:** Windows correctly preserved the operator-approved Unicode SEO filename and FTP/Host UTF-8 transport was healthy, but the Server/Public storage contract still allowed a raw non-ASCII basename to flow through Product/Portfolio media paths. This shared-host boundary must not depend on raw Unicode filename encoding.
**Correct fix:** source commit `b5980306400f3f0951804cab1ad049c1aab4a6d7` adds one deterministic `canonical_server_media_filename()` contract. Existing safe ASCII basenames remain byte-for-byte names; non-ASCII basenames are converted only at Server/Public storage to an ASCII name derived from factual `source_title` plus image slot. Windows metadata, ALT/caption, selected identity and media bytes/SHA remain unchanged. Import, Product media conversion and republish parity all use the same helper.
**Verification:** Unicode Product+Portfolio regression 1/1 PASS; full unified import 5/5 PASS; related visibility/profile/video 21/21 PASS; py_compile, Django check, no migration drift and diff-check PASS. Production deploy/retry remains gated on selective release from exact Production baseline and fresh rollback/readiness verification.
**Prevention:** any filename that crosses FTP/Host/public storage must pass one shared server-basename contract; parity must compare that canonical public name rather than the Windows-local display filename. A failed Bridge import must never be retried unchanged.
...
**Verification:** focused 27/27 PASS. First full gate 69/70 failed only the stale assertion requiring tracking URL in Caption; changed-condition assertion now requires UTM metadata and no raw Caption URL. Full Site+Social rerun 70/70 PASS; final local-media hardening 11/11 PASS; 6 changed Python compile, diff-check and Qt VerifyOnly PASS; Server delta=0. Production acceptance from exact `447e8130...`: #536 existing Feed was reused (no duplicate), automatic Story `6ab383f9497d7707d81648c8` reached Buffer `sent` + live Instagram Story URL; v3 Story asset 1080×1920 public SHA parity PASS; Shop Grid #536 Product destination verified. ERR-49-228 is CLOSED.
**Historical-post caveat:** #536's already-live Feed was intentionally not recreated, so its old v4 raw-URL caption remains. Buffer cannot edit an already-published Instagram post; if visual cleanup is desired, edit that caption directly in Instagram. Future v5 Feed captions are corrected.
...
**Superseded for default route by ERR-49-228:** Buffer mobile remains required only for the optional `native_sticker_notification` mode. The accepted default is now automatic Story + Bio/Shop Grid CTA, proven live on #536 and not dependent on mobile.
**Provider API boundary verified 2026-09-22:** Buffer's public GraphQL exposes `Channel.hasActiveMemberDevice` as a read-only capability and its documented public mutations do not expose mobile-device registration or Test Notification. A schema introspection attempt also returned `Failed to process introspection query`; do not repeat it unchanged. Device registration is relevant only if the operator explicitly selects native-sticker notification mode.
**Prevention:** capability preflight must follow the selected Story mode. Never block the proven automatic Story route on mobile-only requirements belonging to optional native stickers.
