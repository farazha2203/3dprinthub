## ERR-49-235 - #628 still fails after ASCII-safe media basename release; exact stage trace required
**Date:** 2026-09-24
...
**Correction to ERR-49-234:** canonical ASCII Server/Public media basenames are still correct hardening and their dedicated Product+Portfolio regression passes, but they were not the full source of this Production exception. Host locale/filesystem and FTP are UTF-8, and the remaining failure occurs elsewhere in the import/signal/conversion path.
**Next diagnostic:** use repository-owned `scripts/host/phase50_a2z_trace_import.py` against the already-uploaded failed Batch. It changes MEDIA_ROOT to a temporary directory, wraps DB work in an outer atomic transaction with forced rollback, runs each importer stage separately, and prints the real traceback. It must not call the Bridge importer or retry #628. The first `/tmp` launch failed before `django.setup()` with `ModuleNotFoundError: config` because a standalone script uses its own directory as Python import root; no importer stage ran. The harness now requires/resolves `A2Z_PROJECT_ROOT`, validates `manage.py` + `config/__init__.py`, prepends that verified root to `sys.path`, and is compile/diff tested before the changed-condition rerun.
**Prevention:** do not promote a plausible boundary diagnosis to final root cause solely from a matching error string. After a rollback-safe production reproduction persists under a changed condition, capture exact stage/traceback before any further patch or retry.
