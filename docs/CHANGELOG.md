...
- Focused O4R worker coverage proves full-source flags/Profile/Screenshot and first-failure circuit-breaker. First isolated real Sphinx acceptance exposed `source_print_profiles_json=[]`; O4R now explicitly reuses factual MakerWorld `refresh_source_profiles(fresh_capture=True)` before local Profile/Filament import. Expanded O4R/O2H/O2D/O2/O2G/A2W regression 74/74 PASS; py_compile, git diff-check and Qt VerifyOnly PASS.
- O4R final acceptance: fresh SQLite+filesystem rollback; isolated real Sphinx retest PASS with 5 selected Source images, 2 file links, 1 factual Source Profile, 1 imported Sales Profile/Filament row, screenshot and Crawl hide; canonical bounded acceptance then created exactly Product #863 with the same evidence. Post-apply O4R+O2H 10/10 and visible exact-SHA PID 54368 PASS. Production/Host unchanged.
...
- Historical compatibility data roots are excluded from destructive traversal.
