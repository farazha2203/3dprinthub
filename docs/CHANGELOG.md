...
- Focused O4R worker coverage proves full-source flags/Profile/Screenshot and first-failure circuit-breaker. First isolated real Sphinx acceptance then proved Product/images/files/screenshot/Crawl-hide but exposed `source_print_profiles_json=[]`; O4R now explicitly reuses factual MakerWorld `refresh_source_profiles(fresh_capture=True)` before local Profile/Filament import. Expanded O4R/O2H/O2D/O2/O2G/A2W regression 74/74 PASS; py_compile, git diff-check and Qt VerifyOnly PASS. Canonical Catalog and Production unchanged by this development slice.
