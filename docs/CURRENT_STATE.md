...
Exact next microphase O4R2 GitHub gate: final diff/staging review -> commit/push source/tests/docs -> verify Local=Remote -> STOP. O4R3 after that: fresh canonical backup -> isolated real MakerWorld failed-Lamp reacquire acceptance -> exact-SHA Qt/UI smoke -> only if clone proves full data/Profile/Screenshot and successful Product disappearance, bounded canonical owner-visible acceptance. Then resume O5B.
