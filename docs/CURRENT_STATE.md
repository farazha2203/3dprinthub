...
W4.1 extends the factual Source Profile contract to dimensions without inventing missing axes. MakerWorld Description evidence is parsed in source order and bound to Source Profiles only when the number of unambiguous size records equals the number of Source Profiles. Full L×W×H values are normalized to cm. Partial factual values remain partial: the Qt Profile editor shows unknown axes as `نامشخص` instead of `0.00`, while manual Profiles still require complete positive dimensions.

Real Hydra evidence is proven from the latest local MakerWorld capture for Catalog Product #628 / external 3179519. The compact meta text contains `Small Version: Height = 120 mm` and `Large Version: Height = 180 mm2. Support...`; the parser now treats the trailing `2.` as the next numbered section instead of rejecting the `mm` unit. Real-capture acceptance returns exactly two Profiles: instance 3595936 -> Small -> factual height 12cm, and instance 3596024 -> Large -> factual height 18cm. Length/width remain unknown because Source did not provide them.

For Product #625, current Profile 1 already has operator-owned 5×5×5cm and must not be overwritten. Profile 2 has 0/0/0 and no factual Source dimension. The owner-approved fallback is therefore scoped only to still-missing axes and records 4×4×4cm with `dimension_source=owner_estimated` / estimated provenance. Source factual axes always override only the same axis; operator pricing, Filament choices, weight/time and Site identity/revision are not part of this patch.

Rollback ref before W4.1 source work: `backup/pre-phase50-a2w-w41-source-dimensions-20260921` -> `f8a23ab0e903912a3e67c2829f53ceacd1b3c376`. Local verification: W4.1 focused 10/10 PASS; mature Commerce/Profile + W3/W4/W4.1 corrected gate 59/59 PASS; retained media/site 83/83 PASS; py_compile, git diff-check and RUN_QT VerifyOnly PASS. Real Hydra latest-capture parse PASS with exact 12/18cm results.

No real Catalog Product row, Site Product, Host source, migration or Production state has been changed by W4.1 yet. Exact next: commit/push this Local-tested source, verify remote SHA exact, take a fresh integrity-checked Catalog SQLite backup, launch Qt from the exact pushed SHA, then controlled dimension-only patch #628 and #625 with before/after invariants. The failed publish batch `26aa571c-...` remains separate and must not be retried during the dimension patch.
