## 2026-09-21 - Phase50.A.2X PRODUCTION_VERIFIED / CLOSED

A2X runtime is Production-verified on branch `release/phase50-a2j-hero-20260915`; the accepted runtime-bearing commit is `94e53831be67368ec199c1ea5b3ab728cbfff445`. Host worktree is clean, MySQL remains `sfkilvrs_EmiAdmin_3dprinthub`, and migration plan is empty. The dedicated reverse tunnel was used for every Host operation.

The original #620 case-only material parity failure is closed. Controlled retry used new batch `desktop_catalog_v85_20260921_163452` / UUID `2cb55e9e-35fa-40b0-8901-2606e7a4118c` and published Site Product #41 revision 1 plus Slider #17 revision 1 with `republish_parity.ok=true`, zero mismatches, one media row and exactly 160 authoritative active CC Variants; stale active Variants are zero. Windows final state remains `uploaded`, `upload_ready=0`, `needs_update=0`, empty sync error, and no new batch was created during Hero repair.

ERR-49-212 is also closed. Hero #17 no longer persists the intentionally private `store/imported-models` working-media URL. Stored URL is now `https://3dprinthub.ir/media/p/620/6495fc086c41/autumn-apple-3d-print-01.webp`; effective render URL is `/media/p/620/6495fc086c41/autumn-apple-3d-print-01.webp`. The one-time normalization changed exactly one Hero row while keeping Hero revision=1 and ProductCatalogProfile revision=1; Product #620 was not re-published.

Verification is complete: Unified Sync/Hero ownership/import E2E 11/11 PASS; Home/Slicebox/Hero regression 16/16 PASS; compile/diff/Django/no-drift PASS. Production Home/Store/Product return HTTP 200; canonical Hero image returns HTTP 200 and matches Windows SHA. Real Chrome/Playwright at 1440x900 found 3 Hero images, all with nonzero natural dimensions, canonical #620 image present, Product link present, zero private imported-media refs, zero broken images, zero page errors and zero console errors.

Fresh rollback for the final Hero follow-up is `/home/sfkilvrs/3dprinthub-deploy-backups/20260921-165608-phase50-a2x-hero-media-minimal`; exact pre-change runtime file, `.env` and full MySQL gzip all checksum-verify. ERR-49-213 records why the first full-bundle backup attempt hit quota and was replaced by this scoped verified rollback. Exact next project step: start the next explicitly approved Phase50 slice from the current verified repository/Production state; do not re-publish #620 merely for A2X closure.

## 2026-09-21 - A2X material-parity incident history / CLOSED

The failed #620 batch `desktop_catalog_v85_20260921_160339` / UUID `8be482aa-5469-439d-bad3-6dcab3f9f8de` exposed only material-name case mismatches (`pla/petg` versus canonical `PLA/PETG`). Hotfix `143848eeeeac7be8ec64c33ab7aa4c95c9f42165` aligned only material identity comparison with the Store's existing case-insensitive resolver while keeping all other parity fields strict, and carried authoritative Desktop SEO assignment onto the release lineage. Focused Server 6/6 plus compile/diff/Django/no-drift passed before GitHub-first deployment.

The changed condition was then proven by the successful new #620 batch documented above. This historical incident requires no further retry or repair.
...
Fresh pre-activation rollback is now verified at `/home/sfkilvrs/3dprinthub-deploy-backups/20260920-163425-phase50-a2r-manual-payment-activation`; MySQL gzip and SHA256 validation PASS. Production source remains clean at `888af6b4551b2e6b1e5681aab4c3d9610735474a`, and no payment settings row currently exists.

Raw financial destination values are not propagated through repository/logging paths. The owner must enter the approved destination once through authenticated Django Admin. After that, exact next is masked read-back -> explicit activation -> Checkout/manual receipt/Admin-review UAT -> A2R ACCEPTED.
...
The verifier now expects Product.fixed_price=0 whenever current `sales_profiles_json` exists, while still requiring the exact ProductCatalogProfile range and per-Variant facts. A dedicated no-migration hotfix runner is prepared from exact Production baseline `6f55e034...` with source/.env/MySQL rollback evidence and a focused Host regression. The failed receiver transaction did not commit a new Product revision. Exact next: commit/push hotfix -> guarded deploy -> repair the Windows local linkage from the verified pre-attempt Catalog backup -> retry #625 once under the corrected contract -> strict Production read-back -> Buffer Feed+Story.
...
A2M changes the contract from additive merge to authoritative replacement for Product-owned active commerce state. Explicit Windows sales profiles suppress legacy EP49 regeneration, all non-current ProductVariant rows are deactivated, and post-import parity now rejects any extra active Variant by comparing total active count with the current Desktop matrix. Historical rows remain physically present for order/FK rollback safety. No schema migration is required.

Focused Server suite 32/32 PASS; Django check PASS with known CKEditor warning; migration drift none; diff-check PASS. Production remains unchanged at 9ce01fdc until GitHub commit + guarded deploy + fresh rollback backup. Next after deploy is one controlled #625 republish and browser/API read-back proving only the three current profiles remain active.

## 2026-09-19 - Canonical /media/p + manual card-transfer release LOCAL_TESTED
Production was reverified through the dedicated authenticated tunnel as clean `release/phase50-a2j-hero-20260915 @ f13ba8134cf6569c5a95da080bb97077c0cd6745`; MySQL remains `sfkilvrs_EmiAdmin_3dprinthub`. Real #625 import batch `desktop_catalog_v85_20260919_153311` already passed transactional `republish_parity.ok=true`, retained Site Product #39 and advanced Product revision to 6. The remaining public failure is now isolated: canonical Product files are stored under effective Production `MEDIA_ROOT=/home/sfkilvrs/3dprinthub/media` as `p/625/<sha12>/<seo>.webp`, but the DEBUG=False public media allowlist only routed legacy `store/products|categories|seo`, so `/media/p/...` returned HTTP 404.

Local fix extends only the public image allowlist/URL route to `p/`; private `store/imported-models/` remains rejected. Windows verifier is handled separately on the canonical Windows branch and accepts only legacy public Product media plus canonical `/media/p/`. Manual bank-transfer commerce already existed; this release adds the approved dry-run-by-default payment-settings seed and best-effort operator notification after receipt persistence. Production currently has no StorePaymentSettings row, so card transfer is implemented but not configured until post-deploy backup + explicit seed apply.

Verification: compile/check/no-drift PASS; public-media/manual-payment/checkout/unified-import 18/18 PASS; `git diff --check` PASS. Two stale StoreCheckout test fixtures were corrected after they hit migration-seeded `ShippingMethod(code=post)` and the current weighted shipping signature; no Production behavior was changed by those test-only fixes. Exact next: commit/push + rollback ref -> guarded no-migration deploy from GitHub -> public #625 canonical media HTTP verification -> fresh DB backup/readiness -> apply payment settings -> safe read-back -> reconcile Local #625 ACK without re-import -> Instagram Feed+Story -> conservative Host backup retention cleanup.
...
Targeted Server verification: Python compile PASS; Django check PASS with known CKEditor warning only; `makemigrations --check --dry-run` = no changes; five focused pricing/mapping/API regressions PASS. Rollback ref: `backup/pre-phase50-a2l-pricing-engine-20260919 @ ba05c7fc479ae94d4a85676442008e018dbbcc67`. Pricing source is pushed at `a5f02ecef9321cf148451f1b1a60df42511cd7dc`. A dedicated guarded runner `scripts/host/phase50_a2l_pricing_engine_deploy.sh` is LOCAL_TESTED with Bash syntax/diff-check PASS; it enforces baseline `ba05c7fc...`, live GitHub SHA, clean Host, MySQL identity, empty migration plan, publish-readiness, verified source/.env/MySQL backups and post-merge pricing runtime authority. Exact next: commit/push this runner -> restore/verify dedicated reverse tunnel 22024 -> execute exact runner SHA -> owner-default PLA refresh/real #625 republish.
...
Dedicated reverse tunnel `127.0.0.1:22024 -> Host 22224` is healthy and Production is exact clean `release/phase50-a2j-hero-20260915 @ 16e24292d697e79bc6498d44826e191d6da57e23`. MySQL identity/readiness and empty migration plan were previously verified; no schema change is part of this delta.

Owner screenshot/request adds four acceptance requirements: enlarge the Example-4 Slicebox, display real Product SEO copy on the homepage, make Product Hero media/caption navigate to the Product detail page, and make same-identity Windows Republish update/verify every Desktop-owned Product parameter. Real Catalog/Production evidence isolated Product #625 -> Site #39: receiver revision reached 5 and current image bytes/current active Profiles were present, but ACK success was too shallow, Product media filenames churned with random storage suffixes, and explicit manufacturer was collapsed into brand.

Local fix now: Product-backed Hero is authoritative whenever available; wrapper grows to 1280px with reserved 16:9 responsive sizing and CSS cache key 50.7.0; Product meta title/meta description/focus keyword + Product schema are rendered in the Hero and the full caption plus image link to `slide.target_url`. ERR-49-172 adds transactional post-import parity for title/SEO/price range/media basename+SHA/Profile/Variant/material/brand/manufacturer/color/weight/time/dimensions/pricing inputs; any mismatch rolls back and returns failed ACK. Public Product media uses Product-specific content-addressed directories with exact SEO basename and idempotent same-byte reuse.

Verification: Python compile/check/no-model-drift PASS; focused Republish/Hero/Profile/Admin/Visibility suite 37/37 PASS, including exact SEO filename/idempotence, deliberate parity-failure rollback and distinct manufacturer regression. One unrelated stale Filament storefront-template assertion was excluded after its first failure rather than repeated unchanged. Exact next: final diff/runner syntax -> documentation commit/push -> exact-SHA guarded Host deploy -> browser desktop/mobile Hero/link verification -> fresh Catalog+MySQL/media backup -> controlled #625 republish -> read-back parity/public Product verification.
