...
**Exact next:** document/commit/push ERR-49-220 hardening -> Local=GitHub -> exact-SHA Qt. Then owner links/signs in to Buffer mobile for the same account and enables notifications. After that changed condition: read-only Buffer connection/post check -> fresh Catalog backup -> retry #609 once; Feed must return existing receipt/no createPost, Story notification only may create -> require Buffer notification accepted (not error) -> operator completes Instagram Link Sticker handoff -> record truthful Story/Highlight status -> close A2Z-S. Do not switch to automatic no-link Story.
