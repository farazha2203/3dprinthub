...
Exact next: final diff/staging review -> commit/push O3 implementation/tests/checkpoint docs -> verify Local=Remote -> fresh canonical Catalog integrity backup -> exact-SHA Qt runtime verification -> isolated rollback-clone real Product deep-repair acceptance (never the canonical Catalog first) proving same Product ID/Source identity, rollback, fresh local remap, old-local bounded handling, Site/receipt preservation and quick_check. If that passes, document O3 ACCEPTED. Immediately following phase is O4 Delete Semantics Repair.
