...
Requested acquisition UI is now implemented on that lineage: Operations has top-level `موجودی محصولات`, `تک محصول`, `جستجو / لینک جستجو`, and `گزارش و History`. Direct Product URLs use mature adaptive `run_single`; Search URLs/keywords use Preview-first `run_batch`; MakerWorld plain keywords resolve to `/en/search/models?keyword=...`; a Product URL pasted into Search is moved to Single Product instead of entering the listing crawler. Source refresh now belongs to Inventory with independent limits; proven browser/Saved-HTML/method controls remain under Advanced.

Verification PASS: exact O7A flow 5/5; related Operations/Crawl 155/155; Phase50 broad 147/147; Instagram/Buffer/Story 65/65; Category Provider Bridge 3/3; py_compile/compileall/pip/diff-check; Qt VerifyOnly foundation/full-parity/operator launcher. One Professional Commerce profile-identity assertion fails identically on clean entry HEAD `eba70a55`, proving unrelated baseline debt.
...
Owner-visible UI contract was exercised on fresh isolated clone `D:\projects\3dprinthub-backups\phase50-a2z-o6b-ui-smoke-20260926-185322\catalog.sqlite3` to avoid canonical writes. Final corrected harness PASS: Products and `افزودن محصولات / Crawl` routes present; Stage-3 `رفرش رسانه از DB/Local` present; `♻ بازیابی کامل از صفر` present; `🗑 حذف واقعی Crawl` present; independent `📸 ارسال پست Instagram` and `📱 ارسال استوری Instagram` actions present; Product filter contract contains both `instagram_posted` and `instagram_story`.

Product #862 runtime/UI truth remains exact after refresh: DB images=4 / selected=4 / exact selected Local=4 / gallery before refresh=4 / gallery after refresh=4; filenames remain exact `01.webp`..`04.webp`, all pixmaps valid, Site compare disabled and mismatches=[]. Live Story readiness remains provider=buffer / channel=3dprinthub_ir / ready=true / mode=`bio_shop_grid` / requires_mobile_handoff=false / hasActiveMemberDevice=false / blockers=[]. No Story/Post was sent.

Read-only clone integrity PASS: quick_check=ok, Product exact-identity duplicates=0, active Product-backed Crawl=0, duplicate current Social finals=0, stale #536/#625 do not satisfy current-revision Social filters. Final canonical post-smoke read-only snapshot is quick_check=ok / Products=961 / History=4168 / Receipts=533 / Crawl=1177 / Candidates=625, Product exact Source+external-id duplicates=0, Product URL duplicates=0, active Product-backed Crawl=0, current Social final duplicate groups=0. O6B performed no canonical Product/Receipt/Crawl write, no provider mutation, no Instagram publish, no Host/Production change and no migration.

The first two isolated UI-label assertions failed only because the remote harness compared Unicode labels through an unsafe PowerShell stdin literal representation; no application regression was involved. ERR-49-261 records the corrected ASCII-only `\u`/`\U` harness rule. O6B status=ACCEPTED.

Exact next phase O6C: read-only Server delta / lineage audit only — verify current Local/GitHub head, inspect exact file delta from selective Production/Server head, classify Windows-only Catalog/docs versus any Django/Server/static/migration delta, verify reverse-tunnel readiness read-only only if Server evidence is needed, and decide `NO DEPLOY REQUIRED` versus O6D Production Gate. STOP. Immediately following phase: O6D conditional deploy only if O6C proves a real Server delta; otherwise skip directly to O6E final documentation closure.
...
One visible exact-SHA acceptance Catalog Center was launched from `2f11c225...` as PID 47280. After documentation closure, the operator-facing Catalog was relaunched from docs-only head `f2c893f5...` (application/source bytes unchanged from `2f11c225...`) and is currently visible as `3DPrintHub Catalog Center v8.9.11 - Qt 6`, PID 51712; a second process check 10 seconds later confirmed it remained alive. Post-launch canonical quick_check=ok with Products=961 / History=4167 / Receipts=533 / Crawl=1177 / Candidates=625. O6A1B made no Buffer/provider mutation, no real Instagram publish, no Host/Production source change and no migration. O6A1B status=ACCEPTED.

Exact next phase O6B: keep the current operator runtime PID 51712 as the single Catalog instance; verify only the remaining owner-visible Product/Crawl/Media/Social flows on the accepted application/source bytes, including the blue DB/Local refresh button label/behavior and Story action readiness; capture read-only post-smoke integrity; STOP. Immediately following phase O6C = read-only Server delta / lineage audit.
...
O5E acceptance is complete from clean GitHub-exact code head `c5f1deb436ba51fc6eaa2c996dd02e8e776104a9`; selective Server `2b48a593...` remains an accepted ancestor and no Server/migration delta exists. Fresh canonical rollback: `D:\projects\3dprinthub-backups\phase50-a2z-o5e-20260926-145851\catalog-before-o5e.sqlite3`, source/backup quick_check=ok, backup SHA256 `c6b853a79943de2b07a57d96cb074f6ee251ca36ed5a6fc1bcb805c1be7b7e57`, counts Products=867 / Receipts=477 / Crawl=1177 / Candidates=625. Isolated clone: `D:\projects\3dprinthub-o5e-acceptance-20260926-145851`.

Corrected isolated acceptance PASS after resetting the disposable clone from rollback: exactly six final receipts were appended on the clone, 477->483, all from exact persisted provider IDs with `reconcile_match_mode=exact_provider_id` and `reconciled_without_repost=true`: #219 Story `6ab538413e8c1aa3fcce029b`, #219 Feed `6ab537743e8c1aa3fccdedb5`, #301 Feed `6ab550f37cb0c4ddfc0905f8`, #596 Feed `6ab533240516e0ed46cec5e4`, #670 Feed `6ab54fa67cb0c4ddfc08c6e0`, #717 Feed `6ab4ed52a4337780696a01d8`. Provider GraphQL audit counted six read queries and zero mutation queries; no `createPost`/repost occurred.

All 21 non-receipt Catalog tables remained exact versus the pre-O5E manifest, proving zero Product/media/price/Profile/Filament/Site-authority drift. Control receipts #536/#625/#609/#588 remained byte-logically exact. #301 still has no current Story receipt; #588 only has a stale Story-failed receipt and no current Story publication; #609 retained the same current `instagram_story_notification_ready` provider-error evidence and no published Story. Clone quick_check=ok. Canonical Catalog all-table digests, including `sync_receipts`, remained exact versus the pre-O5E manifest with quick_check=ok, so O5E made zero canonical writes.

The first disposable-clone harness run reached the correct six receipt additions but then failed only because the harness guessed the wrong #609 non-final status label. Per ERR-49-257 the clone was reset from the verified rollback and the corrected harness reran to PASS; canonical state was never touched by either attempt. Full acceptance evidence is stored outside the Repository at `D:\projects\3dprinthub-backups\phase50-a2z-o5e-20260926-145851\o5e_acceptance_report.json`.

O5E status = ACCEPTED.

O5F is now ACCEPTED. Entry lineage was clean GitHub-exact `b7594d7b253d9c30ab376b02e199fc4b01f9c2b5` on `wip/phase50-a2z-o5c-reconcile-contract-20260926`; selective Server `2b48a593...` remained an accepted ancestor and no Catalog window was running before the write. The first backup-verifier attempt stopped before mutation due to its own tuple-row helper bug and is documented as ERR-49-258; it was not reused. Fresh authoritative rollback `D:\projects\3dprinthub-backups\phase50-a2z-o5f-20260926-150923\catalog-before-o5f.sqlite3` passed source+backup quick_check and exact all-table logical digest comparison, with SHA256 `c6b853a79943de2b07a57d96cb074f6ee251ca36ed5a6fc1bcb805c1be7b7e57`; pre-write counts remained Products=867 / Receipts=477 / Crawl=1177 / Candidates=625.

Immediately before canonical write, live Buffer exact-ID read-only verification PASS: #219 Feed/Story, #301/#596/#670/#717 Feed all remained `sent`; #609 Story provider `6ab16b959d554f7b28a44302` remained `error`; provider audit observed seven reads and zero mutations. Canonical reconciliation then used only that immediately preceding verified evidence and forbade asset fallback. Exactly six final receipts were appended, Receipts 477->483: #219 Story+Feed and #301/#596/#670/#717 Feed. Every new row carries `reconcile_match_mode=exact_provider_id` and `reconciled_without_repost=true`; no `createPost`, repost, Feed creation or Story creation occurred.

Post-write proof PASS: all 21 non-receipt tables remained exact versus the fresh rollback manifest, so Product/media/price/Profile/Filament/Crawl/Site authority did not drift. #536/#625 stale receipts remained exact, #609 retained current `instagram_story_notification_ready` + provider `error`, #301 still has no current Story receipt and #588 has no current Story publication. Canonical quick_check=ok. Related O5C/O5D Social/Image/duplicate-guard regression reran 77/77 PASS and Qt `RUN_QT.ps1 -VerifyOnly` PASS. Exact-SHA runtime smoke used one launch from `b7594d7b...`; after the initial short handle window the same launch became visible as `3DPrintHub Catalog Center v8.9.11 - Qt 6` PID 57184. Post-launch Catalog remains quick_check=ok at Products=867 / Receipts=483 / Crawl=1177 / Candidates=625. Production/Host and Django migrations changed = NO.

O5F runtime evidence is stored outside the Repository under `D:\projects\3dprinthub-backups\phase50-a2z-o5f-20260926-150923\o5f_canonical_acceptance_report.json`. Exact next microphase O5G is optional owner-selected real provider acceptance only for remaining evidence gaps: #609 clickable Story is still externally blocked by Buffer `hasActiveMemberDevice=false`, while #301/#588 have no current Story provider evidence. O5G must not auto-publish; it requires an explicit owner-selected Product/action if those gaps are to be closed. If no such real provider action is requested, record O5G NOT_REQUIRED/BLOCKED-EXTERNAL as appropriate and proceed to O6A Integrated Local Regression. STOP before O5G.
...
Read-only canonical evidence explains the screenshot instead of assuming a hide bug: visible Lamp examples such as Sphinx #2737114, Mountain Light #2471017, Lotus #1271835, Coral #1682173, GlowShell #1385859, Magic Campfire #2232272, Tripod #2419412, Gothic Arch #2212098 and Squid #2634533 have **no Product row** and their Crawl ledger is `failed / HTTP 403`; therefore they correctly remained because they were never successfully added. A separate Mushroom #1709950 is Product-backed as Product #844 / Crawl collected and the accepted query contract excludes it. Recent successful Products #853..#861 are also Crawl=collected and no longer candidates. UI completion text is now explicit: successful selections disappear; only failed selections remain.

O4R1 adds a distinct multi-select `♻ بازیابی کامل از صفر` action beside the existing non-destructive `بازیابی دیتا + عکس`. For unconsumed Crawl identities it preserves the Source URL/ledger, moves exact active identity folders + Preview out of the active Catalog tree into per-identity rollback quarantine, resets failed attempt state, then re-acquires from the same Product URL with real adaptive fallback, images + downloadable files enabled. After Product creation it imports actual Source Print Profiles into the local Filament/Profile ledger when Source profiles exist and captures a fresh Source screenshot. If exact Product authority already exists, no Product/local bytes are deleted; the Crawl/Candidate truth is reconciled to collected/imported so it disappears from Add Products.

Safety is identity-bounded by exact Source+external-id / normalized URL and current Catalog data root; unrelated folders remain untouched. O4R worker-level changed-condition coverage now proves adaptive full-source acquisition requests images + downloadable files, imports actual Source Print Profiles, captures Screenshot, marks success collected, and stops after the first provider failure leaving later selections untouched. Focused O4R is 5/5 PASS; combined O4R/O2H/O2D/O2/O2G Acquisition/Crawl regression is 68/68 PASS; touched py_compile PASS, Qt VerifyOnly PASS and git diff-check PASS. No canonical Catalog mutation has been performed by this development slice.

O4R2 source/tests/docs were committed/pushed GitHub-exact at `9965e0047db8a2dc9de540d66ce7197cc2f5ec8e`; Local=Remote and Server lineage ancestry PASS. O4R3 first isolated real acceptance used fresh rollback `D:\projects\3dprinthub-backups\phase50-a2z-o4r3-runtime-20260925-234502\catalog-before-o4r3.sqlite3` and clone `D:\projects\3dprinthub-o4r3-acceptance-20260925-234502`. Real failed MakerWorld Crawl #1184 / Sphinx `makerworld:2737114` re-acquired successfully through adaptive fallback `rich -> network_capture`: Product #863 created in clone, 5/5 local Source images saved, 2 file links captured, Source description populated, fresh Source screenshot created, Crawl became collected, candidate disappeared from live Add Products, and clone quick_check=ok. Canonical Catalog remained untouched.

That real gate exposed one missing requested enrichment: generic acquisition returned `source_print_profiles_json=[]`, so the full-clean button could not yet guarantee fresh MakerWorld Profile/Filament recovery. The halted worktree already contained the narrow corrective extension: after Product acquisition, MakerWorld now calls the existing factual `refresh_source_profiles(fresh_capture=True)` contract before `import_source_profiles`; UI reports Source Profile and Profile/Filament separately. Focused O4R 5/5 and expanded related O4R/O2H/O2D/O2/O2G/A2W regression 74/74 PASS; py_compile, diff-check and Qt VerifyOnly PASS. This correction is inspected and accepted for continuation, not discarded.

O4R3B is now complete and accepted. Profile correction/docs are GitHub-exact at code/runtime commit `08e7966d27d1dc1245ab1522ad1c44386f0898b8` with Local=Remote. Fresh rollback `D:\projects\3dprinthub-backups\phase50-a2z-o4r3b-runtime-20260926-130606\catalog-before-o4r3b.sqlite3` has quick_check=ok and SHA256 `c77fc1e4387bb8e42f832332e178b945ab2c09c3e871d9eb2c70e6cee2b972c4`; exact Preview/local targets were separately copied with hashes under `filesystem-before-canonical` before canonical mutation.

Fresh isolated clone `D:\projects\3dprinthub-o4r3b-acceptance-20260926-130606` PASS on real failed Sphinx `makerworld:2737114`: adaptive `rich -> network_capture`, Product #863 in clone, 5 selected Source images / 6 image ledger entries, 2 file links, 1 factual Source Print Profile (`0.2mm layer, 2 walls, 15% infill`), 1 imported Sales Profile/Filament ledger row, fresh screenshot, Crawl collected/live-hidden, unrelated #1185 exact unchanged and quick_check=ok. All five canonical logical digests remained exact after clone acceptance.

Only after clone PASS was one bounded canonical owner-visible acceptance executed for Crawl #1184. Canonical result: exact identity Product #863 created once, 5 selected Source images / 6 image ledger entries, 2 file links, Source description populated, 1 factual Source Print Profile, 1 imported Sales Profile/Filament ledger row, fresh screenshot, Crawl #1184 collected with attempts=0/error cleared and hidden from Add Products. Products 862->863, History 3482->3484, Crawl 1177 unchanged, Candidates 625 unchanged, receipts 477 unchanged, quick_check=ok; pre-mutation SQLite+filesystem rollback and per-identity quarantine both exist. Post-apply O4R+O2H regression 10/10 PASS and Qt VerifyOnly PASS.

Exact-SHA runtime acceptance PASS: old PID 2592 closed gracefully; one launch from `08e7966d...` became visible as `3DPrintHub Catalog Center v8.9.11 - Qt 6` PID 54368. Production/Host and Django migrations changed = NO; this O4R phase is Windows/Catalog-only.

Exact next phase: O5B — read-only Buffer/provider correlation audit from the already-completed O5A receipt inventory. No `createPost`, no Catalog write and no real Instagram publish in O5B. Ordered gate: re-read Social docs/errors -> verify branch/HEAD/clean worktree + Production ancestry -> inspect current Buffer channel readiness -> correlate existing Feed/Story receipts by exact provider post ID first and bounded asset evidence only as fallback -> record ambiguities/missing provider evidence -> docs checkpoint -> STOP. Immediately following phase is O5C reconciliation-contract hardening.
...
Read-only real Catalog audit proved the O4 inconsistency rather than guessing: 852 Products, 191 rejected Product tombstones, and 552 rejected Crawl ledger rows. Product/Crawl status disagrees on 58 existing Product identities. Among rejected Products, 22 stale discovery Candidate rows remain, 16 Preview cache files remain (~172,668 bytes), and four same-identity collected/refetch folders remain. Ten rejected Products have a non-rejected matching ledger status; separately, 18 rejected ledger rows map to active/non-rejected Products. Product authority must win in both directions.

O4 implementation now makes existing Product rows the canonical identity authority before terminal Crawl state. Active/restored Products resolve terminal identity as `collected` even when an older Crawl row says rejected; rejected/blocked Products resolve from Product state. Product Reject now keeps the lightweight Product tombstone/thumbnail/Site+receipt history but purges every bounded same-identity current-data-root folder (base/refresh/refetch/bulk-refetch/deep-repair), matching discovery Candidate and Preview cache, and reports exact cleanup counts/errors to the operator. Product Restore no longer puts an existing tombstone back into Crawl `new`; its ledger becomes `collected`, so the same Product cannot re-enter Add Products. The UI explicitly states that heavy deleted data does not return and O3 `??????? ???? ?? ???` is the Product-specific reacquisition path.

A read-only/apply reconciliation primitive is added for legacy state. It derives desired ledger truth from Product rows (`rejected` / `blocked` / `collected`) and, only for rejected Product tombstones, can purge stale Candidate/Preview/same-identity Local remnants inside the current Catalog data root. Outside-root filesystem targets fail closed. Canonical `apply=False` currently reports 58 ledger mismatches, zero missing rejected ledgers and zero path errors; no canonical O4 mutation has yet been performed.

Verification so far: O4 focused 7/7 PASS; related Product/Crawl/O2D/O2H/O3 lifecycle regression 92/92 PASS; full `test_phase50*.py` 129/129 PASS. py_compile/compileall, pip check, git diff-check, Qt VerifyOnly, Django check and migration drift PASS; known Qt/Pillow deprecation/resource warnings and CKEditor4 warning only. The related gate exposed an old Phase49.3I.38 expectation that rejected Restore should set Crawl `new`; it was updated because O4 intentionally requires `collected`. A completely unrelated Filament manufacturer assertion also failed, and the exact same single test was reproduced on a clean detached baseline `c821a00a`, proving baseline debt rather than O4 regression.

Exact next: final diff/staging review -> commit/push tested O4 source/tests/checkpoint docs -> Local=Remote -> fresh SQLite + exact filesystem rollback of every planned destructive target -> exact-SHA Qt runtime -> destructive reconciliation first on isolated clone/data-root -> if all Product counts/identity/Site/receipt/tombstone/Restore protections pass, apply the bounded reconciliation to canonical Catalog -> post-apply quick_check + zero mismatch audit + O2H Product-protection regression -> O4 closure docs/commit. Then O5 Social Receipt Reconciliation + Operator Acceptance.

## 2026-09-25 - Phase50.A.2Z-O3 Guarded Deep Product Repair - ACCEPTED / O4 NEXT

Active branch: `wip/phase50-a2z-o3-deep-repair-20260925`. O3 implementation/tests/checkpoint docs are GitHub-exact at `1c8db58a9caa147eae2e2c4a734db46a8bfe18da` from baseline `11f2be8931d42e1ccc86377bc99adfe87aa09130`. Production/Server remains unchanged at selective `2b48a593...`; ancestry verification PASS (`2b48a593...` is an ancestor of this forward Windows lineage). O3 is Windows/Catalog-only and introduces no Django model/migration or Host source delta.

Requested delta is now implemented locally: an explicit Product Wizard `??????? ???? ?? ???` action performs a same-identity Source repair only after confirmation. It verifies persisted Source URL -> external identity before any backup/fetch, blocks when operator-owned Product-local media exists, creates and verifies a dedicated full Catalog SQLite rollback (`quick_check=ok` + SHA256) before mutation, fetches into a unique empty `<external>_deep_repair_<token>` folder, revalidates the returned Source identity/local_dir, remaps existing operator-selected Source media by stable URL asset identity, and fails closed if any selected media disappeared.

After successful fresh acquisition, Product identity, operator editorial/commercial decisions, all `server_*` / Site identity fields and the complete sync-receipt set must remain exact. Only then is the former active Product `local_dir` eligible to move into the same repair rollback root as `old_local`; the move is bounded to the current canonical Catalog data root and recognized same-identity folder names. External/legacy compatibility paths are never deletion targets. Failed fresh acquisition removes only its unique partial fresh folder and leaves Product DB/old active Local untouched; the verified rollback remains available.

Changed-condition O3 tests are 6/6 PASS: same-identity successful repair + verified SQLite rollback + old-local quarantine; Source-identity mismatch fail-closed; operator-owned local media fail-closed; partial-acquisition cleanup with Product unchanged; selected Source media disappearance fail-closed; and Product Wizard confirmed action wiring. Related Acquisition/Crawl/O2D/O2E/O2H/Social regression is 87/87 PASS. Full `test_phase50*.py` is 122/122 PASS. Top-level acquisition runtime function inventory proves zero pre-existing function loss. py_compile/compileall, pip check, git diff-check, Qt VerifyOnly, Django check and migration drift all PASS; only the already-known Qt deprecation/temporary-resource warnings in tests and CKEditor4 framework warning remain.

The first related gate correctly exposed one stale pre-O2H UI assertion that still expected `??? ???????`; accepted O2H has separate reversible `?? ???????` plus destructive `??? ?????`, so only that stale regression expectation was synchronized and the unchanged production UI contract then passed. A test-only SQLite inspection handle was also explicitly closed after Windows showed the backup file remained locked during TemporaryDirectory cleanup; runtime backup code already closes its destination connection explicitly.

Runtime/real acceptance is complete. Fresh canonical rollback: `D:\projects\3dprinthub-backups\phase50-a2z-o3-runtime-acceptance-20260925-131601\catalog-before-o3-runtime.sqlite3`; source/backup quick_check=ok, counts match Products=852 / History=3482 / Crawl=1192 / Receipts=477, backup SHA256 `3ae960f10f42d461b17e1de9b9c42d81ab9df71b6465af7f9408d3aedb1c5ed9`. Exact-SHA Qt VerifyOnly PASS and one runtime is visible as Catalog Center v8.9.11 PID 26236.

Real destructive acceptance was performed only on isolated clone `D:\projects\3dprinthub-o3-acceptance-20260925-131753`, using real Product #1 `makerworld:3127131`. Same Product ID/source/external identity, Site authority and receipt set remained exact; `rich` failed over to `network_capture`, 3 images were saved/mapped and all 3 resolve as exact current Product-local media. The per-repair rollback quick_check=ok with SHA256 `a7dfdcf28bb18cd3873005c374f2dcff4e1e20d466cb3bf322bccff3e0e811f3`; clone quick_check=ok; history event is `source_product_deep_repair`. Because the clone row still referenced the canonical old Local path, quarantine correctly returned `skipped_outside_current_data_root`, proving the acceptance run could not move/delete canonical bytes; the focused same-root test separately proves successful `old_local` quarantine. Canonical Catalog remained byte-logically unchanged at digest `6c7b46107c9d77fba16c8460229b0697fa32f35ac0f00634d450e237c4fa18db` with the same four counts and quick_check=ok. O3 is ACCEPTED. Exact next phase: O4 Delete Semantics Repair -> reconcile Product/Crawl queue/cache/tombstone delete behavior and safe Restore without violating O2H Product-authority fail-closed rules.
...
Exact-SHA runtime acceptance PASS on GitHub-exact `bd3ab49f...`. On an isolated clone of the fresh rollback, real failed unconsumed MakerWorld Crawl row #1190 (`makerworld:1385859`) and real Product-backed raw Crawl row #1192 (Product #852, `makerworld:1738908`) were submitted together. Result was exactly deleted=1 / blocked=1 / failed=0: #1190 ledger + Candidate + Preview/local identity data disappeared, while #1192 and Product #852 remained intact. Clone counts changed only Crawl 1192→1191 and Candidates 647→646; Products=852 and History=3482 remained unchanged; clone quick_check=ok. Canonical Catalog stayed untouched at the exact four-table digest `e2993bc95b5fcb89dd67f458156877d469ee4653956095ddcf755f94e641b952`.

Exact-source Qt render on the isolated clone verified separate Reject and Hard Delete controls on both persistent and live-search surfaces. The first render probe used raw Persian literals through the Windows shell and failed only because that diagnostic literal was codepage-mangled; the unchanged source passed immediately with ASCII-safe Unicode escapes. Exact-SHA VerifyOnly PASS. There was no pre-existing Catalog window; one launch was issued. The eight-second handle probe was early, so no second launch was attempted; the same launch became visible after the additional wait as `3DPrintHub Catalog Center v8.9.11 - Qt 6` PID 55596. Post-launch canonical quick_check=ok and counts/digest remain exact: Products=852 / History=3482 / Crawl=1192 / Candidates=647.

Exact next: O2F Instagram all-media/disclosure/source+order links/Story-link capability -> re-read current Buffer/Meta publish path and prior Social errors -> prove why Feed currently collapses to one image despite O2E multi-media authority -> verify provider capabilities for carousel media, AI disclosure, Product details and Story link -> implement only supported contracts -> focused/related Social regression -> fresh rollback where state mutation is possible -> commit/push -> bounded real Post+Story acceptance without duplicate publication. Then O3 guarded deep repair.
...
Corrective source is GitHub-exact at `544e046b62f933e5461d57e9491a5ecee46c5225`: `current_local_items()` synthesizes a canonical selected card from exact finalized Product-local media when the legacy numbered-slot reader omitted that selected URL, and Refresh parity recognizes ASCII-safe Server/Public renames by finalized SHA prefix instead of basename alone. Dedicated omitted-card and Server-rename/SHA regressions PASS; O2E focused 16/16 PASS; direct Instagram publish 13/13 PASS; expanded Image/Product/Social regression 131/131 PASS; continuation related rerun 74/74 PASS; py_compile/compileall, Qt VerifyOnly, Django check, migration drift and diff-check PASS. ERR-49-246 records the #625 UI edge case; ERR-49-247 records the #628 Refresh SHA-parity edge case.

Final corrective rollback is `D:\projects\3dprinthub-backups\phase50-a2z-o2e-final-runtime-acceptance-20260924-201040\catalog-before-o2e-final-runtime.sqlite3`; source/backup quick_check=ok, logical digest exact at `cf21d5ec4605dac1d744f636f438b7a98e625ba8e88982e52466e147d045e890`, counts match at Products=852 / History=3482 / Crawl=1192, backup SHA256 `34fb39594839ef0bfe936f4c88da7ab48b7bd3571220a6991e929355e71da5d9`. Acquisition was quiescent before cutover; latest run #51 was already `completed`.

The old Qt runtime closed through `CloseMainWindow()` with no kill. Exact-SHA VerifyOnly passed and one new visible Catalog Center launched from `544e046b...` as PID 52840. Real #625 acceptance: DB selected=1, Product Wizard selected card=1, Refresh selected-local=1 / Site media=1 / mismatches=0, Social media is exact `/media/p/625/30f41e1d56f5/...`. Real #628 acceptance: DB selected=2, Product Wizard selected cards=2, Refresh selected-local=2 / Site media=2 / mismatches=0, Social media exactly matches SHA prefixes `959abcdd95d6` and `6138e2b6907d`. ProductWizard UI smoke also matched `image_grid.selected_urls()` and primary image for both Products. Target Product+History digest stayed exactly `85d1e5686e1f2dbc8b8c5aea3b6b01188dda5824d726296cc96a2ec5a5bb7fb6` before/after; no Product/Site/Social mutation occurred.

Exact next: O2G Search Crawl Pagination / Target Count -> baseline MakerWorld listing behavior from the real search URL -> make preview/discovery scroll depth target-aware for requested 200/300 -> count only unconsumed identities toward target -> prove continuation past the first ~15 cards or verified exhaustion -> focused/related Crawl regression -> fresh Catalog rollback -> commit/push -> exact-SHA Qt runtime acceptance. Then O2H guarded hard delete -> O2F Instagram disclosure/links -> O3.
...
Real Catalog read-only truth at final Local gate: quick_check=ok; Products=790; discovery ledger=1115; 650 consumed identities hidden; 465 unconsumed identities visible; 47 ready-to-add and 418 incomplete; visible mapped Products=0. Duplicate audit found zero exact external-ID groups, zero normalized-URL groups and zero configured Source-pattern semantic-ID groups. No title-based Product deletion was performed. Final scoped audit digest before/after remained exactly `82a8a906cb4d0d0059dfdf2088cba5457b44116b9c2f6d36e5daa37de4b6c300`.
