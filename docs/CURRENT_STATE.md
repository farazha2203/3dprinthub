...
The Production-based A2L site release carries only the owner-priority web delta: official Tympanus Slicebox Example-4 behavior (`orientation=r`, random cuboids, disperseFactor 30, eager slide loading) and ERR-49-168 Product Admin 504 fix by removing only the unbounded ProductVariant inline. Local Node/Django/no-drift gates PASS and focused Hero+Admin regression is 25/25 PASS. Release `28cac45bf05b09a50fefcaccf3ae43541af022f2` was promoted GitHub-first and its guarded runner PASSed with clean final worktree, MySQL identity/readiness, empty migration plan and rollback root `/home/sfkilvrs/3dprinthub-deploy-backups/20260919-011903-phase50-a2l-site-priority`.

Real Chromium then exposed ERR-49-170: new `static/vendor/slicebox` directories inherited `0700` from deploy `umask 077`, so vendor CSS/JS returned 404 despite correct collected bytes. The corrective release is now limited to deployment/static-permission handling and documentation; no DB/schema/business behavior changes. Exact next: push and execute the permission hotfix, verify nested vendor HTTP/MIME, rerun desktop/mobile Slicebox browser acceptance, then close Production docs.
