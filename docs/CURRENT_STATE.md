...
All stages on #625 are currently locked, including Specs and Commerce. Factual acquisition follows the existing source-sync authority and may refresh `source_print_profiles_json`; Sales Ledger import remains fail-closed while Commerce is locked and must use controlled StageCore unlock after exact-SHA runtime smoke. No Host/Production source, migration, Site revision or publish has changed. Exact next: docs checkpoint -> commit/push W3 -> verify remote exact SHA -> exact-SHA Qt relaunch -> real #625 `دریافت پروفایل از محصول` smoke without publish -> verify two source profiles + Site identity unchanged -> controlled Commerce unlock/import only after that gate.
