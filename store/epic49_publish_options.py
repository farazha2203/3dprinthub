...
def apply_homepage_slider(product, asset, data: dict) -> dict:
    ...
    if requested:
        ...
        if image_row is not None and image_row.image:
            image_url = image_row.image.url
    if not image_url and asset.preview_image:
        image_url = asset.preview_image.url

    defaults = {
        "image_url": image_url,
        "image_alt_text": product.title[:240],
        "title_override": product.title[:220],
        "group_title": getattr(product.category, "name", "")[:160],
        "description": (product.short_description or "")[:480],
...
def sync_epic49_publish_options(asset) -> dict:
    ...
    return {
        ...
        "download_image_limit": _positive_int(data.get("download_image_limit"), 10),
    }
