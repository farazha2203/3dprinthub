...
async function unread(){const badge=document.getElementById("admin-support-unread-badge"),label=document.getElementById("admin-support-unread-label");if(!badge)return;try{const r=await fetch("/admin/website/supportconversation/unread-count/",{headers:{"X-Requested-With":"XMLHttpRequest"},credentials:"same-origin"});if(!r.ok)return;const data=await r.json();const count=Number(data.unread_count??data.count??0);badge.textContent=String(count);badge.hidden=count<1;if(label)label.textContent=count.toLocaleString("fa-IR")+" جدید"}catch(_){}}
