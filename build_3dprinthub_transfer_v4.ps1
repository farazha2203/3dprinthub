...
try:
    print("== 3DPrintHub transfer builder v4 ==")
    ...
    for model in apps.get_models():
        ...
        model_metadata[label] = {
            ...
            "concrete_model": model._meta.concrete_model._meta.label_lower,
    ...
    for label, database_count in sorted(database_counts.items()):
        ...
        # Django dumpdata intentionally does not serialize proxy models.
        # Their manager counts rows from the concrete model table, so comparing
        # those counts against a fixture would create false mismatches.
        if metadata["proxy"] or metadata["auto_created"]:
            skipped_virtual_models.append(
                {
                    "model": label,
            ...
            print(
                f"SKIP_VIRTUAL_MODEL {label}: "
                f"proxy={metadata['proxy']}, "
                f"concrete={metadata['concrete_model']}, "
                f"rows_seen_through_proxy={database_count}"
            )
            continue
    ...
    print("VIRTUAL_MODELS_SKIPPED:", len(skipped_virtual_models))
    print("EXPORT_COUNT_MISMATCHES:", len(mismatches))

    if mismatches:
        fail(
            "Fixture counts do not match the concrete SQLite models. "
    ...
    manifest = {
        ...
        "model_metadata": dict(sorted(model_metadata.items())),
        "virtual_models_skipped": skipped_virtual_models,
