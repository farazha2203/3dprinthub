...
EXPECTED_BASELINE="0c9d328299a77c26fdef9450d985276178ecc120"
TARGET_SHA="${1:-}"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_ROOT="/home/sfkilvrs/3dprinthub-deploy-backups/${STAMP}-phase50-a2r-resume"
TMP_DELTA="/tmp/3dprinthub-a2r-resume-$$.txt"

cleanup(){ rm -f "$TMP_DELTA" 2>/dev/null || true; }
trap cleanup EXIT
fail(){
  printf 'A2R_RESUME_FAIL=%s\n' "$1" >&2
...
if connection.vendor!="mysql" or str(connection.settings_dict.get("NAME") or "")!=expected:
    raise SystemExit("A2R_RESUME_FAIL=database_identity_mismatch")
executor=MigrationExecutor(connection)
plan=executor.migration_plan(executor.loader.graph.leaf_nodes())
print("MIGRATION_PLAN="+repr([(m.app_label,m.name,b) for m,b in plan]))
if plan:
    raise SystemExit("A2R_RESUME_FAIL=migration_plan_not_empty")
ready=publish_readiness()
print("PUBLISH_READY="+repr(ready.get("ready")))
if ready.get("ready") is not True:
    raise SystemExit("A2R_RESUME_FAIL=receiver_not_ready")
PY
...
while IFS= read -r changed; do
  case "$changed" in
    docs/CHANGELOG.md|docs/CURRENT_STATE.md|docs/DEPLOYMENT.md|docs/ERRORS.md|docs/REQUESTS.md|docs/ROADMAP.md|docs/phases/PHASE50_A2R_PAYMENT_FINANCE_ADMIN_CLOSURE.md|scripts/host/phase50_a2r_payment_finance_admin_deploy.sh|scripts/host/phase50_a2r_payment_finance_admin_resume.sh)
      ;;
    *) fail "unexpected_resume_delta:$changed" ;;
  esac
done < "$TMP_DELTA"
mkdir -p "$BACKUP_ROOT"
...
"$PY" -m py_compile store/management/commands/phase50_a2l_seed_manual_payment.py website/templatetags/admin_console.py website/phase50a_admin_command_center.py
"$PY" manage.py check
"$PY" manage.py makemigrations --check --dry-run
...
plan=executor.migration_plan(executor.loader.graph.leaf_nodes())
if plan:
    raise SystemExit("A2R_RESUME_FAIL=post_merge_migration_plan_not_empty")
if publish_readiness().get("ready") is not True:
    raise SystemExit("A2R_RESUME_FAIL=post_merge_receiver_not_ready")
manual=StorePaymentSettings.objects.order_by("pk").first()
manual_configured=bool(
    manual and str(getattr(manual,"account_holder","") or "").strip()
    and (
...
if operator is None:
    raise SystemExit("A2R_RESUME_FAIL=no_active_superuser_for_readonly_render_smoke")
...
for marker in ('id="phase50-payment-readiness-title"', "Merchant credential", "Provider:"):
    if marker not in body:
        raise SystemExit("A2R_RESUME_FAIL=admin_readiness_marker_missing")
for forbidden in ("STORE_PAYMENT_CARD_NUMBER","STORE_PAYMENT_SHEBA_NUMBER","STORE_PAYMENT_ACCOUNT_NUMBER","ZARINPAL_MERCHANT_ID"):
    if forbidden in body:
        raise SystemExit("A2R_RESUME_FAIL=secret_identifier_rendered")
...
headers={"User-Agent":"3DPrintHub-A2R-Resume/1.0","Cache-Control":"no-cache"}
for path in ("/","/store/"):
    with request.urlopen(request.Request(base+path,headers=headers),timeout=20) as r:
        print("PUBLIC_HTTP="+str(r.status)+" "+path)
        if r.status != 200:
            raise SystemExit("A2R_RESUME_FAIL=public_http_failed")
...
printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT"
printf 'PHASE50_A2R_PAYMENT_FINANCE_ADMIN_RESUME=PASS\n'
