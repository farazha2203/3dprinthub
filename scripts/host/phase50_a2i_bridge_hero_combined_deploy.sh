...
BACKUP_ROOT="$BACKUP_BASE/${STAMP}-phase50-a2i-bridge-hero"
TMP_DELTA="/tmp/3dprinthub-a2i-delta-$$.txt"
STATIC_LIST="/tmp/3dprinthub-a2i-static-$$.txt"

cleanup() { rm -f "$TMP_DELTA" "$STATIC_LIST" 2>/dev/null || true; }
trap cleanup EXIT
fail() {
  printf 'PHASE50_A2I_DEPLOY_FAIL=%s\n' "$1" >&2
  printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT" >&2
  exit 1
}

printf '%s\n' "============================================================"
printf '%s\n' "3DPrintHub Phase50.A.2I + A2G Bridge Production Deploy"
...
if connection.vendor != "mysql": raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=database_vendor_not_mysql")
if str(connection.settings_dict.get("NAME") or "") != expected: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=database_name_mismatch")
required={
...
if missing: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=required_migrations_missing")
...
if plan: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=current_migration_plan_not_empty")
ready=publish_readiness(); print("PUBLISH_READY="+repr(ready.get("ready"))); print("PUBLISH_BLOCKERS="+repr(ready.get("blockers") or []))
if ready.get("ready") is not True: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=receiver_not_ready_before_deploy")
PY
"$PY" manage.py check
...
while IFS= read -r path; do
  ...
  case "$path" in
    ...
    scripts/host/phase50_a2i_bridge_hero_combined_deploy.sh) ;;
    static/css/phase50-a2i-slicebox-hero.css) ;;
    static/js/phase49_2c-home-hero.js|static/js/phase50-a2i-slicebox-hero.js) ;;
    templates/website/partials/hero.html) ;;
    website/test_phase49_2c_hero_studio.py|website/test_phase50_a2i_slicebox_hero.py) ;;
...
if plan: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=post_merge_migration_plan_not_empty")
...
if ready.get("ready") is not True: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=receiver_not_ready_after_merge")
PY
...
if len(token)<24: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=bridge_token_missing_after_restart")
def fetch(path, auth=False, limit=2_000_000):
    headers={"User-Agent":"3DPrintHub-Phase50.A.2I-Verify/1.0","Accept":"*/*","Cache-Control":"no-cache"}
...
checks["product"]=fetch("/api/catalog-bridge/v1/products/15/",True)
...
checks["engine_js"]=fetch("/static/js/phase49_2c-home-hero.js?v="+target)
for key,(status,ctype,_) in checks.items(): print(key.upper()+"_HTTP="+str(status)+" CONTENT_TYPE="+ctype)
if any(status!=200 for status,_,_ in checks.values()): raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=http_not_200")
health=json.loads(checks["health"][2].decode("utf-8"))
ready=json.loads(checks["ready"][2].decode("utf-8"))
if health.get("status")!="ok": raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=bridge_health_not_ok")
if ready.get("ready") is not True: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=publish_readiness_not_ready")
product_payload=json.loads(checks["product"][2].decode("utf-8"))
hero_payload=json.loads(checks["hero_api"][2].decode("utf-8"))
product=product_payload.get("product") or {}
if int(product.get("id") or 0)!=15: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=site_product_15_missing")
images=product.get("images") or []
print("PRODUCT15_IMAGE_COUNT="+str(len(images)))
if len(images)<2: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=site_product_15_gallery_missing")
for idx,row in enumerate(images,1):
    url=str(row.get("url") or "").strip()
    print("PRODUCT15_IMAGE_%d_URL=%s"%(idx,url))
    if "/media/store/imported-models/" in url: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=private_imported_media_exposed")
    if not url.startswith("/media/store/products/"): raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=product_media_not_product_owned")
    status,ctype,_=fetch(url)
    print("PRODUCT15_IMAGE_%d_HTTP=%s CONTENT_TYPE=%s"%(idx,status,ctype))
    if status!=200 or "image/webp" not in ctype.lower(): raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=product_media_http_or_type_invalid")
slides=hero_payload.get("items") or []
for row in slides:
    selected=str(row.get("selected_image_url") or "").strip()
    if "/media/store/imported-models/" in selected: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=hero_private_imported_media_exposed")
for key,marker in (("hero_css",b"Phase50.A.2I"),("hero_js",b"P50SliceboxHero"),("engine_js",b"P50SliceboxHero.play")):
    if marker not in checks[key][2]: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=live_static_marker_missing:"+key)
home=checks["home"][2]
for marker in (b"data-p50i-slicebox",b"phase50-a2i-slicebox-hero.css",b"phase50-a2i-slicebox-hero.js"):
    if marker not in home: raise SystemExit("PHASE50_A2I_DEPLOY_FAIL=home_hero_marker_missing")
print("A2I_BRIDGE_HERO_PUBLIC_VERIFY=PASS")
PY
...
printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT"
printf '%s\n' "PHASE50_A2I_BRIDGE_HERO_DEPLOY=PASS"
