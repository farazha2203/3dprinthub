...
EXPECTED_BASELINE="6569e5a9ec7e75da7185b851fc6eea82cd7fc0ea"
TARGET_SHA="${1:-}"
BACKUP_BASE="/home/sfkilvrs/3dprinthub-deploy-backups"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_ROOT="$BACKUP_BASE/${STAMP}-phase50-orderable-publish-contract"
TMP_DELTA="/tmp/3dprinthub-orderable-publish-delta-$$.txt"

cleanup() { rm -f "$TMP_DELTA" 2>/dev/null || true; }
trap cleanup EXIT
fail() {
  printf 'PHASE50_ORDERABLE_PUBLISH_DEPLOY_FAIL=%s\n' "$1" >&2
  printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT" >&2
  exit 1
}

printf '%s\n' "============================================================"
printf '%s\n' "3DPrintHub Phase50 Orderable Publish Contract Hotfix Deploy"
...
while IFS= read -r path; do
  ...
  case "$path" in
    ...
    catalog_center/tests/test_phase49_3i49_site_bulk_publish.py) ;;
    store/phase49_catalog_visibility.py|store/phase50_orderability.py|store/phase50_variant_views.py) ;;
    store/test_phase49_catalog_visibility.py|store/test_phase50_variant2_gallery.py) ;;
    scripts/host/phase50_orderable_publish_contract_deploy.sh) ;;
...
grep -Fq 'variant_is_orderable' store/phase50_variant_views.py || fail "variant_api_shared_orderability_missing"
grep -Fq 'orderable_variant' store/phase49_catalog_visibility.py || fail "visibility_orderable_gate_missing"
grep -Fq 'canonical' catalog_center/app/phase49_3i49_site_publish.py || fail "desktop_canonical_profile_gate_missing"
...
def fetch(path, auth=False, limit=1_000_000):
    headers={"User-Agent":"3DPrintHub-Orderable-Publish-Hotfix/1.0","Accept":"*/*","Cache-Control":"no-cache"}
...
print("ORDERABLE_PUBLISH_PUBLIC_VERIFY=PASS")
PY
...
printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT"
printf '%s\n' "PHASE50_ORDERABLE_PUBLISH_CONTRACT_DEPLOY=PASS"
