...
EXPECTED_BASELINE="9ce01fdc4c61ce58730e77bbb4d70b78de07e585"
TARGET_SHA="${1:-}"
...
trap cleanup EXIT
fail(){
  printf 'A2M_REPUBLISH_DEPLOY_FAIL=%s\n' "$1" >&2
...
if connection.vendor!="mysql" or str(connection.settings_dict.get("NAME") or "")!=expected:
    raise SystemExit("A2M_REPUBLISH_DEPLOY_FAIL=database_identity_mismatch")
...
if plan: raise SystemExit("A2M_REPUBLISH_DEPLOY_FAIL=migration_plan_not_empty")
ready=publish_readiness()
print("PUBLISH_READY="+repr(ready.get("ready")))
if ready.get("ready") is not True: raise SystemExit("A2M_REPUBLISH_DEPLOY_FAIL=receiver_not_ready")
PY
...
for required in store/phase50_profile_matrix.py store/epic49_publish_options.py store/phase50_republish_contract.py store/test_phase50_profile_matrix.py scripts/host/phase50_a2m_authoritative_republish_deploy.sh; do
...
plan=MigrationExecutor(connection).migration_plan(MigrationExecutor(connection).loader.graph.leaf_nodes())
if plan: raise SystemExit("A2M_REPUBLISH_DEPLOY_FAIL=post_merge_migration_plan_not_empty")
if publish_readiness().get("ready") is not True:
    raise SystemExit("A2M_REPUBLISH_DEPLOY_FAIL=post_merge_receiver_not_ready")
p=Product.objects.filter(pk=39).first()
if p is not None:
    q=ProductVariant.objects.filter(product=p)
    print("PRODUCT39_ACTIVE_PRE_REPUBLISH="+str(q.filter(is_active=True).count()))
...
printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT"
printf 'PHASE50_A2M_AUTHORITATIVE_REPUBLISH_DEPLOY=PASS\n'
