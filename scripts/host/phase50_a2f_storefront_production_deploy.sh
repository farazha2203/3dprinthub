...
EXPECTED_BASELINE="e12fdaf281f7e08013e54c7cf936f8275127ab2b"
STATIC_ROOT="/home/sfkilvrs/public_html/static"
TARGET_SHA="${1:-}"

BACKUP_BASE="/home/sfkilvrs/3dprinthub-deploy-backups"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_ROOT="$BACKUP_BASE/${STAMP}-phase50-a2f-storefront"
TMP_DELTA="/tmp/3dprinthub-a2f-delta-$$.txt"
...
fail() {
    printf 'PHASE50_A2F_DEPLOY_FAIL=%s\n' "$1" >&2
    printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT" >&2
    exit 1
}
printf '%s\n' "============================================================"
printf '%s\n' "3DPrintHub Phase50.A.2F Storefront Production Deploy"
...
if connection.vendor != "mysql":
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=database_vendor_not_mysql")
if str(connection.settings_dict.get("NAME") or "") != expected_db:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=database_name_mismatch")
applied = set(
...
if missing:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=required_migrations_missing")

executor = MigrationExecutor(connection)
plan = executor.migration_plan(executor.loader.graph.leaf_nodes())
print("CURRENT_MIGRATION_PLAN=" + repr([(m.app_label, m.name, b) for m, b in plan]))
if plan:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=current_migration_plan_not_empty")

ready = publish_readiness()
print("PUBLISH_READY=" + repr(ready.get("ready")))
print("PUBLISH_BLOCKERS=" + repr(ready.get("blockers") or []))
if ready.get("ready") is not True:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=receiver_not_ready_before_deploy")
PY
...
if plan:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=post_merge_migration_plan_not_empty")
PY
...
if ready.get("ready") is not True:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=receiver_not_ready_after_merge")
PY
...
if len(token) < 24:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=bridge_token_missing_after_restart")

def fetch(path, *, auth=False, limit=2_000_000):
    headers = {"User-Agent": "3DPrintHub-Phase50.A.2F-Verify/1.0", "Accept": "*/*", "Cache-Control": "no-cache"}
    if auth:
...
if home_status != 200 or store_status != 200:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=public_http_not_200")
if health_status != 200 or health.get("status") != "ok":
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=bridge_health_not_ok")
if ready_status != 200 or ready.get("ready") is not True:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=publish_readiness_not_ready")
if js_status != 200 or b"GUIDED_DIMENSIONS" not in js_body or b"guidedCandidates" not in js_body:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=guided_js_not_live")
if css_status != 200 or b"store-profile-step-number" not in css_body:
    raise SystemExit("PHASE50_A2F_DEPLOY_FAIL=guided_css_not_live")
PY
...
printf 'BACKUP_ROOT=%s\n' "$BACKUP_ROOT"
printf '%s\n' "PHASE50_A2F_PRODUCTION_DEPLOY=PASS"
