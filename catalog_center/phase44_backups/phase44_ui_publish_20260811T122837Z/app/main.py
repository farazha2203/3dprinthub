...
HOST_MIRROR=ROOT/"3dprinthub-houst"
...
def load_config():
    ...
    for latest_source in latest.get("sources",[]):
        ...
        merged={**previous,**latest_source}
        merged_sources.append(merged)
    current={**current,**latest,"sources":merged_sources}
    CONFIG_FILE.write_text(
...
class App(tk.Tk):
    ...
    def _style(self):
        ...
        st.configure("TNotebook.Tab",padding=(16,9),background="#e8eef4",foreground=fg,font=("Tahoma",10,"bold"))
        st.map("TNotebook.Tab",background=[("selected",accent)],foreground=[("selected","#071827")])
        st.configure("TButton",padding=(10,7),background="#e2e8f0",foreground=fg,borderwidth=0)
        st.map("TButton",background=[("active","#cbd5e1")])
        st.configure("Primary.TButton",padding=(12,8),background=accent,foreground="#071827",font=("Tahoma",10,"bold"))
        st.map("Primary.TButton",background=[("active","#e2b84f")])
        st.configure("Success.TButton",padding=(12,8),background="#16a34a",foreground="white",font=("Tahoma",10,"bold"))
        st.configure("Warning.TButton",padding=(12,8),background="#f59e0b",foreground="#422006",font=("Tahoma",10,"bold"))
        st.configure("Danger.TButton",padding=(12,8),background="#ef4444",foreground="white")
    ...
    def _upload_ui(self):
        ...
        ttk.Button(bar,text="آپلود آخرین Batch و Import",command=self.upload_last_batch).pack(side="left",padx=4)
    ...
    def _settings_ui(self):
        self.ai_provider=tk.StringVar(value=self.db.setting("ai_provider", self.config.get("ai",{}).get("provider","auto")))
        if self.ai_provider.get() not in {"auto","avalai","openai"}: self.ai_provider.set("auto")
        self.ai_model=tk.StringVar(value=self.db.setting("ai_model", self.config.get("ai",{}).get("model","")))
        ...
        self.google_key=tk.StringVar(value=self.db.setting("google_api_key"))
        ...
        self.ftp_host=tk.StringVar(value=self.db.setting("ftp_host","ftp.3dprinthub.ir"))
        self.ftp_port=tk.StringVar(value=self.db.setting("ftp_port","21"))
        self.ftp_user=tk.StringVar(value=self.db.setting("ftp_user","sfkilvrs"))
        self.ftp_password=tk.StringVar()
        self.ftp_remote_root=tk.StringVar(value=self.db.setting("ftp_remote_root","/3dprinthub"))
        self.site_url=tk.StringVar(value=self.db.setting("site_url","https://3dprinthub.ir"))
        self.bridge_token=tk.StringVar()
        ...
        ttk.Label(help_box,text=r"فایل‌های D:\projects\3DPrintHub\APIKEY-AVAL.txt و APIKEY.txt در صورت وجود فقط روی همین Windows خوانده می‌شوند و می‌توانی آنها را به Credential Store منتقل کنی.",style="SubHeader.TLabel",wraplength=1050).pack(anchor="w")
    ...
    def _refresh_ai_key_source(self):
        ...
        for provider in self._provider_candidates():
            parts.append(f"{provider}: {provider_key_source(provider)}")
    ...
    def _selected_ai_provider(self):
        ...
        for provider in ("avalai","openai"):
            if get_provider_key(provider): return provider
        return "avalai"

    def _ai_key(self,provider=None):
        provider=provider or self._selected_ai_provider()
        return self.ai_key.get().strip() or get_provider_key(provider)
    ...
    def _selected_batch_images(self,row):
        all_urls=self._safe_json_list(row["images_json"])
        selected=self._safe_json_list(row["selected_images_json"])
        primary=row["primary_image_url"] or ""
        if not selected:
        ...
        mapping={}
        extract_file=src/"page_extract.json"
        if extract_file.is_file():
            try:
                payload=json.loads(extract_file.read_text(encoding="utf-8"))
                for item in payload.get("images",[]):
                    if isinstance(item,dict) and item.get("url") and item.get("local_file"):
                        p=Path(item["local_file"])
                        if p.is_file():mapping[item["url"]]=p
            except Exception:pass
        local_files=sorted([p for p in (src/"images").glob("*") if p.is_file()]) if (src/"images").is_dir() else []
        for url in all_urls:
            if str(url).startswith("local://"):
                candidate=src/"images"/str(url).split("local://",1)[1]
                if candidate.is_file():mapping[url]=candidate
        if not mapping:
            for idx,url in enumerate(all_urls):
                if idx < len(local_files):mapping[url]=local_files[idx]
        return [(url,mapping.get(url)) for url in selected]
    ...
    def build_batch(self):
        ...
        batch=BATCH_ROOT/name; models=batch/"models"; models.mkdir(parents=True,exist_ok=True)
        manifest=[]; batched_ids=[]
        for r in rows:
            target=models/f"{r['source_code']}_{r['external_id']}"; target.mkdir(parents=True,exist_ok=True)
            ...
            selected_urls=[url for url,_ in selected_pairs if url]
            image_target=target/"images"; copied=0; local_image_files=[]
            for index,(url,local_file) in enumerate(selected_pairs,start=1):
                local_name=""
                if local_file and Path(local_file).is_file():
                    suffix=Path(local_file).suffix.lower() or ".jpg"
                    image_target.mkdir(parents=True,exist_ok=True)
                    local_name=f"{index:03d}{suffix}"
                    shutil.copy2(local_file,image_target/local_name)
                    copied+=1
                local_image_files.append(local_name)
        ...
        (batch/"batch_manifest.json").write_text(json.dumps({"schema_version":"8.5","batch_uuid":batch_uuid,"batch_name":name,"models":manifest},ensure_ascii=False,indent=2),encoding="utf-8")
        ...
        messagebox.showinfo(APP,f"Batch v8.5 ساخته شد:\n{batch}\n\nمحصولات: {len(manifest)}\nBatch UUID: {batch_uuid}")
    ...
    def _refresh_connection_secret_source(self):
        if hasattr(self,"connection_secret_source"):
            self.connection_secret_source.set(
                f"رمز FTP: {secret_source('ftp_password')} | توکن Bridge: {secret_source('bridge_token')}"
            )

    def _site_connection(self,require_bridge=True):
        password=self.ftp_password.get().strip() or get_secret("ftp_password")
        token=self._entered_bridge_token() or get_secret("bridge_token")
