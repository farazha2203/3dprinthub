...
Write-Host "3DPrintHub Catalog Intelligence LEGACY / ROLLBACK" -ForegroundColor Yellow
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Legacy release verification failed" }
    & $Python "$Root\launch.py"
    if ($LASTEXITCODE -ne 0) { throw "Legacy application exited with code $LASTEXITCODE" }
} finally {
