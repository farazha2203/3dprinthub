...
CONTENT_SCHEMA = {
    ...
    "properties": {
        "title_fa": {"type": "string"},
        "short_description_fa": {"type": "string"},
        "description_fa": {"type": "string"},
...
def validate_content_pack(
    result: dict[str, Any],
    source_title: str = "",
    operator_title_fa: str = "",
) -> dict[str, Any]:
    """Reject generic/empty output and enforce an explicit operator identity."""
    ...
    canonical = str(operator_title_fa or "").strip()
    if canonical:
        if _normalize_title(title) != _normalize_title(canonical):
            raise RuntimeError(
                "هوش مصنوعی عنوان قطعی اپراتور را رعایت نکرد. هیچ محتوایی ذخیره نشد؛ دوباره اجرا کن یا Provider/Model را تغییر بده."
            )
        # Preserve the operator's exact spelling/spacing as the saved H1 identity.
        result["title_fa"] = canonical
    elif str(source_title or "").strip() and len(title) < 6:
...
class AIContentService:
    ...
    def test_connection(self) -> str:
        result = self.client.test_connection(self.model)
        return (
            f"{result['provider_label']} connected | model={result['model']} | "
            f"models={result['models_count']} | sample={result['sample']}"
        )
    ...
    def enrich_product(
        ...
        payload = {
            ...
            "operator_title_fa": source.get("operator_title_fa") or "",
            ...
            "license_name": source.get("license_name") or "",
            "source_price": source.get("source_price"),
            "source_currency": source.get("source_currency") or "",
            "estimated_weight_grams": source.get("estimated_weight_grams"),
            "estimated_print_minutes": source.get("estimated_print_minutes"),
        ...
        for url in (image_urls or [])[:4]:
            if str(url).startswith(("http://", "https://")):
                content.append({"type": "input_image", "image_url": str(url), "detail": "auto"})

        strict_translate = str(mode or "commerce").lower() == "translate"
        canonical = str(payload["operator_title_fa"] or "").strip()
        operator_rule = ""
        if canonical:
            operator_rule = (
                "OPERATOR CANONICAL IDENTITY OVERRIDES TRANSLATION: operator_title_fa is an explicit human-approved Persian product name. "
                "You MUST return title_fa EXACTLY equal to operator_title_fa, with no prefix, suffix, synonym, transliteration, model number, or retranslation. "
                "Every short/full/use description, SEO title, SEO description, sales bullet, social caption, tag, keyword, image alt text and homepage slider copy must refer to the same product using this canonical identity consistently. "
                "Do not reintroduce an older/wrong scraped title anywhere in generated Persian editorial text. "
            )
        ...
        instructions = (
            ...
            + operator_rule
            + "SOURCE TITLE IS AUTHORITATIVE PRODUCT IDENTITY when no operator_title_fa is supplied. title_fa must faithfully translate the concrete object, use, theme/character/style, and meaningful modifiers from source_title into natural Persian. "
            ...
            "similar_persian_keywords are editorial hints collected from previously reviewed products in the same local category. Reuse or adapt only phrases that are semantically relevant to this product; never treat them as product facts, never copy irrelevant phrases, and never let them override the current source facts or operator_title_fa. "
            ...
            + (
                "Use faithful translation; do not add marketing claims. "
                if strict_translate
                else "Write persuasive but factual Persian ecommerce copy. "
            )
        )
