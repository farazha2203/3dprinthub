...
def write_minimal_package(source: Path) -> None:
    ...
    (source / "PACKAGE_MANIFEST.json").write_text(
        json.dumps({"version": "8.5.4"}), encoding="utf-8"
    )
    (source / "app" / "main.py").write_text("VERSION='new'\n", encoding="utf-8")
    (source / "app" / "site_connection.py").write_text("# new\n", encoding="utf-8")
    (source / "app" / "version.py").write_text('APP_VERSION = "8.5.4"\n', encoding="utf-8")
    (source / "launch.py").write_text('EXPECTED_VERSION = "8.5.4"\n', encoding="utf-8")
...
class V85UpgradeTests(unittest.TestCase):
    ...
    def test_install_preserves_data_and_rollback_restores_previous_state(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = install(source, target, data, backups, stamp="20260810-180000")
            self.assertTrue((target / "app" / "main.py").is_file())
            with closing(sqlite3.connect(db_path)) as connection:
                self.assertEqual(
                    connection.execute("SELECT value FROM sample").fetchone()[0], "before"
                )
            with closing(sqlite3.connect(db_path)) as connection:
    ...
    def test_install_works_when_extracted_package_is_nested_inside_old_target(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            target = root / "installed"
            source = target / "3DPrintHub_Catalog_Intelligence_v8.5.4"
            write_minimal_package(source)
            (target / "old-root.txt").write_text("old", encoding="utf-8")

            result = install(source, target, root / "data", root / "backups", stamp="20260810-181000")

            self.assertEqual((target / "app" / "version.py").read_text(encoding="utf-8"), 'APP_VERSION = "8.5.4"\n')
    ...
    def test_install_can_verify_and_stage_an_already_extracted_target_root(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            target = root / "installed"
            write_minimal_package(target)

            result = install(target, target, root / "data", root / "backups", stamp="20260810-182000")
    ...
    def test_invalid_batch_is_rejected_before_network_connection(self):
        with tempfile.TemporaryDirectory() as temporary:
            batch = Path(temporary) / "bad-name"
            batch.mkdir()
            (batch / "batch_manifest.json").write_text(
                json.dumps({"schema_version": "8.5", "batch_name": "bad-name", "batch_uuid": "uuid"}),
                encoding="utf-8",
            )
